// Costco Business Center Westminster - Interactive Scripts
...
contactForm.addEventListener('submit', function(e) {
    ...
    const subject = encodeURIComponent(`Inquiry - ${data.orderType || 'General'} - Costco Business Center`);
    const body = encodeURIComponent(
        `Name: ${data.name}\n` +
        `Phone: ${data.phone}\n` +
        `Email: ${data.email || 'N/A'}\n` +
        `Inquiry Type: ${data.orderType || 'N/A'}\n` +
        `Party Size/Quantity: ${data.partySize || 'N/A'}\n` +
        `Details: ${data.message || 'N/A'}\n\n` +
        `Please contact me regarding my inquiry.`
    );

    window.location.href = `mailto:info@costcobusinesscenter.com?subject=${subject}&body=${body}`;
...
document.querySelectorAll('a[href^="tel:"]').forEach(link => {
    link.addEventListener('click', () => {
        if (typeof gtag === 'function') {
            gtag('event', 'click_to_call', { business: 'Costco Business Center' });
        }
    });
});
